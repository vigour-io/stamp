...
exports.after = function (stamp, fn) {
  if (!after[stamp]) { after[stamp] = [] }
  after[stamp].push(fn)
}

// @todo: do perf tests check if its faster to do force hashtable on the start
exports.close = function (stamp) {
  const listeners = stamp in on && on[stamp]
  const afterListeners = stamp in after && after[stamp]
  if (listeners) {
